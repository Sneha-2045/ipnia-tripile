...
const TravelDocumentsPage = () => {
  ...
  // Domestic India↔India: passport step is not part of the flow
  useEffect(() => {
    if (!isInternational && state.selectedFlight) {
      navigate("/booking/hotel", { replace: true });
    }
  }, [isInternational, state.selectedFlight, navigate]);
  ...
  const origin = state.selectedFlight?.originCity || state.selectedFlight?.origin || "origin";
  const destination =
    state.selectedFlight?.destinationCity || state.selectedFlight?.destination || "destination";
  ...
  return (
    <BookingPageShell
      ...
      subtitle="Passport details are required because this journey includes a point outside India."
    >
      <SEO
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
        <div className="space-y-6">
          <section className="rounded-2xl border border-[#d4a853]/30 bg-[#0c1a2e] p-5">
            <p className="text-sm text-white/80">
              This booking is international ({origin} → {destination}). Please enter passport details
              exactly as they appear on the travel document.
            </p>
          </section>
